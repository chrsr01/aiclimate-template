...
from aiclimate.exceptions import ConfigDoesNotExistException, InvalidConfiguration
...
DEFAULT_CONFIG = {
    'templates_dir': os.path.expanduser('~/.aiclimate/templates/'),
    'replay_dir': os.path.expanduser('~/.aiclimate/replay/'),
...
def get_config(config_path: Path | str) -> dict[str, Any]:
    ...
    raw_templates_dir = config_dict['templates_dir']
    config_dict['templates_dir'] = _expand_path(raw_templates_dir)
...
def get_user_config(
    ...
    Otherwise look up the config file path in the ``AICLIMATE_CONFIG``
    ...
    try:
        ...
        env_config_file = os.environ['AICLIMATE_CONFIG']
    except KeyError:
