...
from aiclimate import __version__
from aiclimate.config import get_user_config
from aiclimate.exceptions import (
...
def version_msg() -> str:
    """Return the aiclimate version, location and Python powering it."""
    python_version = sys.version
    location = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return f"aiclimate {__version__} from {location} (Python {python_version})"
...
def _builtin_template_path() -> str:
    """Return path to the built-in project template."""
    return str(Path(__file__).parent / '_builtin_template')


def list_installed_templates(
    default_config: bool | dict[str, Any], passed_config_file: str | None
) -> None:
    """List installed (locally cloned) templates. Use aiclimate --list-installed."""
    config = get_user_config(passed_config_file, default_config)
    templates_folder: str = config['templates_dir']
    if not os.path.exists(templates_folder):
        click.echo(
            f"Error: Cannot list installed templates. "
            f"Folder does not exist: {templates_folder}"
        )
        sys.exit(-1)

    template_names = [
        folder
        for folder in os.listdir(templates_folder)
        if os.path.exists(
            os.path.join(templates_folder, folder, 'aiclimate.json')
        )
        or os.path.exists(
            os.path.join(templates_folder, folder, 'cookiecutter.json')
        )
    ]
...
@click.option(
    '--no-input',
    is_flag=True,
    help='Do not prompt for parameters and only use template config file content. '
...
@click.option(
    '--directory',
    help='Directory within repo that holds the template config file '
...
def main(
    ...
    """Create a project from a template (TEMPLATE).

    aiclimate is a project scaffolding tool for AI in climate and environmental
    sciences research. Run without arguments to use the built-in template.
    """
    ...
    # Use built-in template when no template argument is provided
    if not template:
        template = _builtin_template_path()
    elif template.lower() == 'help':
    ...
    try:
        generate_project(
            ...
            password=os.environ.get('AICLIMATE_REPO_PASSWORD'),
            directory=directory,
