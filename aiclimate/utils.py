"""Helper functions used throughout aiclimate."""
...
# Valid context keys for template configuration files.
CONTEXT_KEYS = ('aiclimate', 'cookiecutter')


def get_context_key(context: dict[str, Any]) -> str:
    """Return the context key ('aiclimate' or 'cookiecutter') from a context dict."""
    for key in CONTEXT_KEYS:
        if key in context:
            return key
    msg = f"Context must contain one of {CONTEXT_KEYS} as a key"
...
def create_tmp_repo_dir(repo_dir: Path | str) -> Path:
    ...
    base_dir = tempfile.mkdtemp(prefix='aiclimate')
...
def create_env_with_context(context: dict[str, Any]) -> StrictEnvironment:
    ...
    try:
        ...
        ctx_key = 'aiclimate'
    envvars = context.get(ctx_key, {}).get('_jinja2_env_vars', {})
