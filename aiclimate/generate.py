...
from aiclimate.find import find_template
from aiclimate.hooks import run_hook_from_repo_dir
from aiclimate.prompt import YesNoPrompt
from aiclimate.utils import (
...
def is_copy_only_path(path: str, context: dict[str, Any]) -> bool:
    ...
    """
    try:
        ctx_key = next(k for k in ('aiclimate', 'cookiecutter') if k in context)
        for dont_render in context[ctx_key]['_copy_without_render']:
...
def generate_file(
    ...
    ctx_key = next(k for k in ('aiclimate', 'cookiecutter') if k in context)
    if context[ctx_key].get('_new_lines', False):
        # Use `_new_lines` from context, if configured.
        newline = context[ctx_key]['_new_lines']
...
def _run_hook_from_repo_dir(
    ...
    warnings.warn(
        ...
        "use 'aiclimate.hooks.run_hook_from_repo_dir' instead",
        DeprecationWarning,
        2,
    )
