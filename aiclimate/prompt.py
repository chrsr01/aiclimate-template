...
from aiclimate.exceptions import UndefinedVariableInTemplate
from aiclimate.utils import create_env_with_context, get_context_key, rmtree
...
def prompt_for_config(
    ...
    prompts = context[ctx_key].pop('__prompts__', {})

    # First pass: Handle simple and raw variables, plus choices.
    # These must be done first because the dictionaries keys and
    # values might refer to them.
    count = 0
    all_prompts = context[ctx_key].items()
    ...
    for key, raw in context[ctx_key].items():
...
def choose_nested_template(
    ...
    prompts = context[ctx_key].pop('__prompts__', {})
    key = "templates"
    config = context[ctx_key].get(key, {})
    if config:
        # Pass
