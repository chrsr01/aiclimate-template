...
from aiclimate import zipfile
from aiclimate.exceptions import InvalidZipRepository
...
def test_unzip_local_file(mocker, clone_dir) -> None:
    ...
    mock_prompt_and_delete = mocker.patch(
        'aiclimate.zipfile.prompt_and_delete', return_value=True, autospec=True
    )
...
def test_unzip_protected_local_file_user_password(mocker, clone_dir) -> None:
    ...
    mocker.patch('aiclimate.zipfile.read_repo_password', return_value='sekrit')
...
def test_unzip_protected_local_file_user_bad_password(mocker, clone_dir) -> None:
    ...
    mocker.patch(
        'aiclimate.zipfile.read_repo_password', return_value='not-the-right-password'
    )
...
def test_unzip_url(mocker, clone_dir) -> None:
    ...
    mocker.patch(
        'aiclimate.zipfile.requests.get',
...
def test_unzip_should_abort_if_no_redownload(mocker, clone_dir) -> None:
    ...
    mocker.patch(
        'aiclimate.zipfile.prompt_and_delete', side_effect=SystemExit, autospec=True
    )
...
def test_unzip_is_ok_to_reuse(mocker, clone_dir) -> None:
    ...
    mock_prompt_and_delete = mocker.patch(
        'aiclimate.zipfile.prompt_and_delete', return_value=False, autospec=True
    )
